...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function formatStory(story, episodeNumber = null) {
        ...
        if (storyState.isEpisodic && episodeNumber < storyState.totalEpisodes) {
            formattedStory += `<div class="episode-transition">
                <p><em>Ese episode ikurikira bizagenda gute? ntuzacikwe</em></p>
            </div>`;
        }
...
function TextGenerator(prompt, episodicOptions = null) {
  ...
  `"${mainCharacter.name} na ${secondaryCharacter.name}": Urukundo n’Ibanga mu ${storyTheme.theme}`,
  `Urugendo rwa ${mainCharacter.name}: ${capitalizeFirstLetter(conflict)} yahinduye amateka`,
  `"${capitalizeFirstLetter(storyTheme.theme)}": Inkuru ishingiye ku buzima bwa ${mainCharacter.name}`,
  `Iyo ${mainCharacter.name} yahuye na ${secondaryCharacter.name}: Inkuru y’umunsi utazibagirana`,
  `"${capitalizeFirstLetter(conflict)}": Ibyago, Inzozi n’Icyizere bya ${mainCharacter.name}`,
  `${mainCharacter.name} – Mu marembera y’icyizere muri ${setting.place}`,
  `Mu izina ry'urukundo: Inkuru ya ${mainCharacter.name} na ${secondaryCharacter.name}`,
  `"Ntawe ubyiruka atazi icyo ahunga" – ${mainCharacter.name} mu rugendo rudasanzwe`,
  `Amarira y’icyumweru: ${mainCharacter.name} n’inkovu z’ahahise`,
  `Hagati y’Ukuri n’Urukundo: ${mainCharacter.name} ahanganye n’ukuri kwa ${secondaryCharacter.name}`
];
  ...
  `Byari ${setting.weather} ku ${setting.time}, ubwo ${mainCharacter.name} yari ${setting.place}.`,
  `${mainCharacter.name} yari umuntu {descriptions}, ariko icyo gihe ibintu byose byari bigiye guhinduka.`,
  `${setting.place} ni ho inkuru ya ${mainCharacter.name} itangirira.`,
  `Icyo gihe, ${mainCharacter.name} yari afite imyaka ${mainCharacter.age}, maze ahura na ${secondaryCharacter.name} bwa mbere.`,
  `Abantu benshi bo muri ${setting.place} bamenye ${mainCharacter.name} {timeExpressions}.`,
  `${secondaryCharacter.name} yari {descriptions} cyane, kandi uko bahuye si ibisanzwe.`,
  `Hari umugani uvuga ngo "${getRandomItem(kinyarwandaDict.proverbs)}"… ibi byabaye kuri ${mainCharacter.name}.`,
  `${mainCharacter.name} yahoranaga {emotions} mu mutima, ariko uwo munsi wari ugiye guhindura byose.`,
  `Mu myaka yashize, ubwo byose byatangiraga, ${mainCharacter.name} ntiyari azi ko ubuzima bwe buzahinduka gutyo.`,
  `Inkomoko y’inkuru ya ${mainCharacter.name} ni iyuzuye amayobera, ariko niho byose byatangiriye...`
];
  ...
  // Gusoza urugendo rw'inkuru
  `Nyuma y'imyaka myinshi, ${mainCharacter.name} yashoboye kumenya impamvu y'ibyo byose, aramenya n’icyo agomba kubahoho.`,
  `${mainCharacter.name} yagaruye amahoro mu buzima bwe, amenya ko {cultural} bifite agaciro karenze ibyo yatekerezaga.`,
  
  // Guhinduka kw’imitima
  `${mainCharacter.name} na ${secondaryCharacter.name} baricaye baganira ku {conflict}, bagera ku bwumvikane n’ubwiyunge bw’ukuri.`,
  `Mu gihe gito, ubuzima bwa ${mainCharacter.name} bwahindutse burundu; ntiyongera kuba umuntu {descriptions} yari asanzwe ari.`,
  `${mainCharacter.name} yamenye ko {emotions} bifite uburemere kuruta {emotions}, maze atekana ku mutima.`,
  
  // Inyigisho cyangwa amagambo
  `${secondaryCharacter.name} yagize ati: "{conversations}", abivuga yishimye kubera uburyo bari babanye.`,
  ...
  // Gufata inshingano no kuba urugero
  ...
  // Kwerekana igisubizo cy'amarira
  `Nubwo hari byinshi yabuze, ${mainCharacter.name} yasanze agaciro k’ubuzima kari mu kubabarira no gukomeza urugendo.`,
  `${mainCharacter.name} ntiyari akiri umuntu {descriptions} yari asanzwe ari, yari amaze kuba mushya mu bitekerezo no mu mutima.`
];
